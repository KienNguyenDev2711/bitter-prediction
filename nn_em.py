...
data_train["sentences"] = data_train["mol"].apply(lambda x: MolSentence(mol2alt_sentence(x, radius=2)))
...
data_phyto["sentences"] = data_phyto["mol"].apply(lambda x: MolSentence(mol2alt_sentence(x, radius=2)))
...
data_unimi["sentences"] = data_unimi["mol"].apply(lambda x: MolSentence(mol2alt_sentence(x, radius=2)))
...
def main():
    ...
    history = model.fit(x_train, y_train, validation_data=(x_phyto, y_phyto), epochs=15, batch_size=128)
    ...
    # model.save("model/")
    return (f1_score(phyto_pred.round(), y_phyto), average_precision_score(y_phyto, phyto_pred))
...
while (1):
    ...
    print("max aupr", aupr)
    if ((score[0] >= 0.93) and (score[1] >= 0.95)): break
