...
bi = {}
train_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(m, radius=3, bitInfo= bi, nBits=256) for m in train_mol]
train_fps_vec = []
for fp in train_fps:
    arr = np.array([])
...
bi = {}
phyto_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(m, radius=3, bitInfo= bi, nBits=256) for m in phyto_mol]
phyto_fps_vec = []
for fp in phyto_fps:
    arr = np.array([])
    DataStructs.ConvertToNumpyArray(fp, arr)
...
bi = {}
unimi_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(m, radius=3, bitInfo= bi, nBits=256) for m in unimi_mol]
unimi_fps_vec = []
for fp in unimi_fps:
    arr = np.array([])
    DataStructs.ConvertToNumpyArray(fp, arr)
...
def main():
    model = Sequential([
        Dense(256, input_shape=(256, ), activation="relu", kernel_regularizer=l2(0.001)),
        Dropout(0.3),
    ...
    # model.save("model/Paper_ecfp_nn2")
...
while (1):
    ...
    print("max aupr", aupr)
    if ((score[0] >= 0.8) and (score[1] >= 0.75)): break
