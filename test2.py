...
bi = {}
train_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(m, radius=2, bitInfo= bi, nBits=512) for m in train_mol]
train_fps_vec = []
for fp in train_fps:
    arr = np.array([])
...
bi = {}
phyto_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(m, radius=2, bitInfo= bi, nBits=512) for m in phyto_mol]
phyto_fps_vec = []
for fp in phyto_fps:
    arr = np.array([])
    DataStructs.ConvertToNumpyArray(fp, arr)
...
bi = {}
unimi_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(m, radius=2, bitInfo= bi, nBits=512) for m in unimi_mol]
unimi_fps_vec = []
for fp in unimi_fps:
    arr = np.array([])
    DataStructs.ConvertToNumpyArray(fp, arr)
...
def main():
    model = Sequential([
        Dense(256, input_shape=(512, ), activation="relu", kernel_regularizer=l2(0.001)),
        Dropout(0.3),
    ...
    x_phyto = unimi_fps_vec
    y_phyto = data_unimi["Bitter"].values.reshape(-1, 1)

    model.compile(optimizer=keras.optimizers.Adam(lr=0.001), loss=tf.keras.losses.BinaryCrossentropy(from_logits=False), metrics=[f1])
    history = model.fit(x_train, y_train, validation_data=(x_phyto, y_phyto), epochs=10, batch_size=64)
    ...
    return (f1_score(unimi_pred.round(), y_unimi), average_precision_score(y_unimi, unimi_pred))
